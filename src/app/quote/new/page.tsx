...
import {
  ChevronRight, ChevronLeft, Search, Sparkles, Check,
...
export default function NewQuotePage() {
  ...
  const [newClient, setNewClient] = useState({
    name: "", email: "", phone: "", location: "", techLevel: "beginner", clientType: "residential", notes: "",
  });
  ...
  async function runResearch() {
    ...
    try {
      const res = await fetch("/api/research", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          serviceType, location: selectedClient.location, description,
          techLevel: selectedClient.techLevel, needLevel,
      ...
      {step === 1 && (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          ...
          <div className="relative mb-3">
            ...
            <input className="input pl-9" placeholder="Search by name or location..." value={search}
              ...
              <button key={c.id} onClick={() => setSelectedClient(c)}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500">
                    ...
                    {c.quotes.length > 0 && ` · ${c.quotes.length} previous quote${c.quotes.length !== 1 ? "s" : ""}`}
                  </p>
                </div>
                {selectedClient?.id === c.id && <Check className="w-4 h-4 text-blue-600 flex-shrink-0" />}
              </button>
            ))}
          </div>
            ...
            <div className="border border-slate-200 rounded-lg p-4 bg-slate-50 space-y-3 mb-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input className="input" placeholder="City, State" value={newClient.location}
                ...
                <div>
                  ...
                  <input className="input" type="tel" value={newClient.phone}
                    onChange={(e) => setNewClient((p) => ({ ...p, phone: e.target.value }))} />
                </div>
                <div>
                  <label className="label">Tech Level</label>
                  <select className="input" value={newClient.techLevel}
                    onChange={(e) => setNewClient((p) => ({ ...p, techLevel: e.target.value }))}>
                    {Object.entries(TECH_LEVEL_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
                  </select>
                </div>
                <div>
                  <label className="label">Client Type</label>
                  <select className="input" value={newClient.clientType}
              ...
              <button onClick={createClient} disabled={!newClient.name || !newClient.location || creatingClient}
          ...
          {selectedClient && selectedClient.quotes.length > 0 && (
            <div className="border-t border-slate-100 pt-4">
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Previous Work with {selectedClient.name}</p>
              <div className="space-y-1.5">
                {selectedClient.quotes.slice(0, 4).map((q, i) => (
                  <div key={i} className="flex items-center gap-3 text-xs bg-amber-50 border border-amber-100 rounded-lg px-3 py-2">
                    <span className="font-medium text-slate-700">{SERVICE_LABELS[q.serviceType] || q.serviceType}</span>
                    <span className="text-slate-400">—</span>
                    <span className="font-semibold text-slate-900">
                      {q.finalPrice ? formatCurrency(q.finalPrice) : q.recommendedPrice ? formatCurrency(q.recommendedPrice) : "No price set"}
                    </span>
                    <span className="text-slate-400 ml-auto">{new Date(q.createdAt).toLocaleDateString()}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-5">
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {SERVICE_OPTIONS.map(([value, label]) => (
                  <button key={value} onClick={() => setServiceType(value)}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input className="input" type="number" min="0.5" step="0.5" placeholder="e.g. 2.5"
                  value={estimatedHours} onChange={(e) => setEstimatedHours(e.target.value)} />
                <p className="text-xs text-slate-400 mt-1">Optional — helps AI give a better price</p>
              </div>
              <div>
                <label className="label">Urgency / Need Level</label>
                <select className="input" value={needLevel} onChange={(e) => setNeedLevel(e.target.value)}>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          ...
          {researchLoading && (
            <div className="flex flex-col items-center py-14 gap-3">
              ...
              <p className="text-sm text-slate-500">Researching rates for {selectedClient?.location}...</p>
              <p className="text-xs text-slate-400">Checking market data for {SERVICE_LABELS[serviceType]}</p>
            </div>
          )}
          ...
          {researchError && !researchLoading && (
            <div className="flex items-start gap-3 p-4 bg-red-50 border border-red-200 rounded-lg mb-4">
              ...
              <div>
                ...
                <button onClick={runResearch} className="text-xs text-red-700 underline mt-2">Try again</button>
              </div>
            </div>
          )}
          ...
          {research && !researchLoading && (
            <div className="space-y-4">
              <div className="bg-gradient-to-br from-slate-50 to-blue-50 rounded-xl p-5 border border-blue-100">
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-4">Market Rate Range — {selectedClient?.location}</p>
              ...
              {myRate && (
                <div className="bg-emerald-50 rounded-lg p-3 border border-emerald-200">
                  ...
                  <p className="text-sm text-emerald-800">{formatCurrency(myRate.min)} – {formatCurrency(myRate.max)} / {myRate.unit}</p>
                </div>
              )}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input className="input pl-7 text-lg font-semibold" type="number" value={recommendedPrice}
                    onChange={(e) => { setRecommendedPrice(e.target.value); setFinalPrice(e.target.value); }} />
                </div>
              </div>
            </div>
          )}
          ...
          {!researchLoading && (
            <div className="flex justify-between mt-6">
              ...
              <button onClick={() => setStep(4)} disabled={!research}
                className="btn-primary flex items-center gap-2">
      ...
      {step === 4 && (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          ...
          <div className="bg-slate-50 rounded-lg p-4 border border-slate-200 mb-5 grid grid-cols-2 gap-2 text-sm">
            <div><span className="text-slate-500">Client:</span> <span className="font-medium">{selectedClient?.name}</span></div>
            <div><span className="text-slate-500">Location:</span> <span className="font-medium">{selectedClient?.location}</span></div>
            <div><span className="text-slate-500">Service:</span> <span className="font-medium">{SERVICE_LABELS[serviceType]}</span></div>
            <div><span className="text-slate-500">Need Level:</span> <span className="font-medium capitalize">{needLevel}</span></div>
            {estimatedHours && <div><span className="text-slate-500">Est. Hours:</span> <span className="font-medium">{estimatedHours}h</span></div>}
            {research && (
              <div><span className="text-slate-500">AI Recommended:</span>{" "}
                <span className="font-semibold text-blue-600">{formatCurrency(research.recommended)}</span>
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input className="input pl-7 text-xl font-bold" type="number" placeholder="0"
            ...
            <div>
              ...
              <textarea className="input" rows={3} placeholder="Notes about scope, client preferences, what was agreed..."
          ...
          <div className="flex justify-between mt-6">
            ...
            <button onClick={saveQuote} disabled={!finalPrice || saving}
              className="btn-primary flex items-center gap-2">
              {saving ? <><Loader2 className="w-4 h-4 animate-spin" /> Saving...</> : <><Check className="w-4 h-4" /> Save Quote</>}
            </button>
          </div>
        </div>
      )}
